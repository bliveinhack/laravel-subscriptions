...
**bliveinhack Subscriptions** is a flexible plans and subscription management system for Laravel, with the required tools to run your SAAS like services efficiently. It's simple architecture, accompanied by powerful underlying to afford solid platform for your business.

[![Packagist](https://img.shields.io/packagist/v/bliveinhack/laravel-subscriptions.svg?label=Packagist&style=flat-square)](https://packagist.org/packages/bliveinhack/laravel-subscriptions)
[![Scrutinizer Code Quality](https://img.shields.io/scrutinizer/g/bliveinhack/laravel-subscriptions.svg?label=Scrutinizer&style=flat-square)](https://scrutinizer-ci.com/g/bliveinhack/laravel-subscriptions/)
[![Travis](https://img.shields.io/travis/bliveinhack/laravel-subscriptions.svg?label=TravisCI&style=flat-square)](https://travis-ci.org/bliveinhack/laravel-subscriptions)
[![StyleCI](https://styleci.io/repos/93313402/shield)](https://styleci.io/repos/93313402)
[![License](https://img.shields.io/packagist/l/bliveinhack/laravel-subscriptions.svg?label=License&style=flat-square)](https://github.com/bliveinhack/laravel-subscriptions/blob/develop/LICENSE)
...
1. Install the package via composer:
    ```shell
    composer require bliveinhack/laravel-subscriptions
    ```

2. Publish resources (migrations and config files):
    ```shell
    php artisan bliveinhack:publish:subscriptions
    ```

3. Execute migrations via the following command:
    ```shell
    php artisan bliveinhack:migrate:subscriptions
    ```
...
**bliveinhack Subscriptions** has been specially made for Eloquent and simplicity has been taken very serious as in any other Laravel related aspect. To add Subscription functionality to your User model just use the `\bliveinhack\Subscriptions\Traits\HasPlanSubscriptions` trait like this:

```php
namespace App\Models;

use bliveinhack\Subscriptions\Traits\HasPlanSubscriptions;
...
```php
$plan = app('bliveinhack.subscriptions.plan')->create([
    'name' => 'Pro',
...
```php
$plan = app('bliveinhack.subscriptions.plan')->find(1);
...
$amountOfPictures = app('bliveinhack.subscriptions.plan_feature')->where('slug', 'pictures_per_listing')->first()->value;

// Get feature value through the subscription instance
$amountOfPictures = app('bliveinhack.subscriptions.plan_subscription')->find(1)->getFeatureValue('pictures_per_listing');
```
...
```php
$plan = app('bliveinhack.subscriptions.plan')->find(2);
$subscription = app('bliveinhack.subscriptions.plan_subscription')->find(1);
...
```php
// Find plan feature
$feature = app('bliveinhack.subscriptions.plan_feature')->where('name', 'listing_duration_days')->first();
...
$subscriptions = app('bliveinhack.subscriptions.plan_subscription')->byPlanId($plan_id)->get();

// Get bookings of the given user
$user = \App\Models\User::find(1);
$bookingsOfSubscriber = app('bliveinhack.subscriptions.plan_subscription')->ofSubscriber($user)->get(); 

// Get subscriptions with trial ending in 3 days
$subscriptions = app('bliveinhack.subscriptions.plan_subscription')->findEndingTrial(3)->get();

// Get subscriptions with ended trial
$subscriptions = app('bliveinhack.subscriptions.plan_subscription')->findEndedTrial()->get();

// Get subscriptions with period ending in 3 days
$subscriptions = app('bliveinhack.subscriptions.plan_subscription')->findEndingPeriod(3)->get();

// Get subscriptions with ended period
$subscriptions = app('bliveinhack.subscriptions.plan_subscription')->findEndedPeriod()->get();
```

### Models

**bliveinhack Subscriptions** uses 4 models:

```php
bliveinhack\Subscriptions\Models\Plan;
bliveinhack\Subscriptions\Models\PlanFeature;
bliveinhack\Subscriptions\Models\PlanSubscription;
bliveinhack\Subscriptions\Models\PlanSubscriptionUsage;
```
...
- [Chat on Slack](https://bit.ly/bliveinhack-slack)
- [Help on Email](mailto:help@bliveinhack.com)
- [Follow on Twitter](https://twitter.com/bliveinhack)
...
If you discover a security vulnerability within this project, please send an e-mail to [help@bliveinhack.com](help@bliveinhack.com). All security vulnerabilities will be promptly addressed.
...
(c) 2016-2022 bliveinhack LLC, Some rights reserved.
